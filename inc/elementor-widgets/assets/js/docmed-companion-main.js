/**
 * Docmed Companion widgets without jQuery: carousels, the portfolio filter,
 * scroll-to-top and counters, on the ColorlibUI bundle the Docmed theme loads.
...
(function () {
  ...
  function run() {
    ...
    if (window.imagesLoaded) {
      UI.imagesLoaded('.docmed-portfolio', function () {
        // filter items on button click
        UI.toElements('.portfolio-menu').forEach(function (menu) {
          menu.addEventListener('click', function (e) {
            var item = e.target.closest && e.target.closest('p');
            if (!item || !menu.contains(item)) return;
            UI.isotope('.docmed-portfolio', { filter: item.getAttribute('data-filter') });
          });
        });
        // init Isotope
        UI.isotope('.docmed-portfolio', {
    ...
    // MC Scripts
    if (document.querySelector('.docmed-subscribe-newsletter-area')) {
