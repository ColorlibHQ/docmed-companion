 ...
 * puts them in .docmed-portfolio-load and lays the Isotope grid out again.
 * No jQuery.
 */
(function () {
  'use strict';
    ...
    buttons.forEach(function (button) {
      button.addEventListener('click', function (e) {
        ...
        UI.request(settings.action_url, { method: 'POST', data: data }).then(function (html) {
          UI.toElements('.docmed-portfolio-load').forEach(function (el) {
            el.innerHTML = html;
          });

          UI.isotope('.docmed-portfolio', 'reloadItems');
